...
import { setting } from './switchPage.js';
...
class SettingOptions {
    ...
    init() {
        if (!settingsDB.has(this.options.dataKey).value()) {
            ...
            if (!Array.isArray(settingsDB.get(this.options.dataKey).value())) {
                settingsDB.set(this.options.dataKey, []).write();
            }
        }
    }
    ...
    addOptions() {
        // 所有 input 失去焦点
        for (let i = 0; i < this.options.nodes.inputElement.length; i++) {
    ...
    deleteOptions() {
        ...
        listElement.addEventListener('click', (e) => {
            ...
            if (target.classList.contains('delete')) {
                ...
                settingsDB.get(this.options.dataKey).splice(liIndex, 1).write();
...
class SettingServeOptions extends SettingOptions {
    ...
    addOptions() {
        ...
        this.options.nodes.addElement.addEventListener('click', () => {
            ...
            const priceElementValue = this.options.nodes.priceElement.value;

            // 如果名称为空
            if (!nameElementValue) {
                this.options.nodes.hintElement.style.display = 'flex';
                this.options.nodes.hintElement.innerHTML = '请输入项目名称';
                this.options.nodes.nameElement.value = '';
...
class SettingTypeOptions extends SettingOptions {
    ...
    addOptions() {
        ...
        this.options.nodes.addElement.addEventListener('click', () => {
            ...
            // 更新数据库
            settingsDB.get(this.options.dataKey).push(nameElementValue).write();
...
(() => {
    ...
    const locationElement = optionsElement.querySelector('.form .location');
    ...
    changeElement.addEventListener('click', () => {
        ...
        if (!dialogRest.canceled) {
            locationElement.value = path.join(dialogRest.filePaths[0], appName);
...
(() => {
    ...
    const passwordElement = optionsElement.querySelector('.form .password');
